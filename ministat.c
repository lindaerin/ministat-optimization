...
#include "dtoa/strtod-lite.c"												// to optimize strtod -> atof -> strtod-fast.			double strtod_fast(const char *str, char **endptr)

#include <fcntl.h> 															// declared for open read call
#include <time.h>
#include <pthread.h>
...
unsigned long long int t_elap[2];
int count = 0;
...
static void
AddPoint(struct dataset *ds, double a, int timing_flag)
{
	...
	// 	t_elap[count] = elapsed_us(&start, &end);
	// 	count++;
	// }
}

static void
Append(struct dataset *ds1, struct dataset *ds2)
{
    ds1->points = realloc(ds1->points, (sizeof *ds1->points * ds1->n) + (sizeof *ds2->points * ds2->n) );
	memcpy(&ds1->points[ds1->n],ds2->points, (sizeof *ds2->points * ds2->n));
	ds1->n += ds2->n;
	ds1->sy += ds2->sy;
    ds1->syy += ds2->syy;
	free(ds2);
...
void *read_store(void *argument){
	struct work * args = (struct work *) argument; 
	const char * n = args -> file; 
	int seek_start = args -> start;
	int bytes_to_read = args -> end- seek_start; 
	struct dataset *s; 
	int timing_flag= args->flag_v;
...
static struct dataset *
ReadSet(const char *n, int column, const char *delim, int timing_flag)
{	
	...
	double split_size= stats.st_size/num_of_threads;
	struct work assignment[num_of_threads];
	pthread_t thread_id[num_of_threads]; 

	double current_byte=0;
	int extra_read = 0;

	if(timing_flag==1)
    	clock_gettime(CLOCK_MONOTONIC, &start);

	//assigns thread their working portion of file
	for(int i=0;i<num_of_threads;++i){
		assignment[i].file= n;
		assignment[i].start= current_byte;
		assignment[i].end= split_size + current_byte-extra_read;
		assignment[i].flag_v= timing_flag;

		//checks if last number is cut short
		char buf[1];

		f = open(n, O_RDWR);
		lseek(f, assignment[i].end - 1, SEEK_SET);
		extra_read = 0; 
		...
		memset(buf, 0,1);
		assignment[i].end += extra_read;
		current_byte = assignment[i].end;
		close(f);

		pthread_create(&thread_id[i], NULL, read_store, (void *)&assignment[i]);
	}
...
usage(char const *whine)
{
	...
	fprintf(stderr, "\t-v : optimization timing)\n");
...
int main(int argc, char **argv)
{
	...
	if(flag_v==1){
      ...
      t_elap[count] = elapsed_us(&start, &end);
      count++;
	  printf("\nTotal Readset elapsed time = %llu nanoseconds\n", t_elap[0]);
	  printf("Main elapsed time = %llu nanoseconds\n", t_elap[1]);
	}
